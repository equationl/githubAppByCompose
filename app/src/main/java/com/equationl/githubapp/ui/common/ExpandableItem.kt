 ...
 *
 * @param content 主内容
 * @param subContent 点击后展开的内容
 * @param modifier modifier
 * */
@Composable
fun ExpandableItem(
    content: @Composable RowScope.(isExpand: Boolean) -> Unit,
    subContent: @Composable ColumnScope.(isExpand: Boolean) -> Unit,
    modifier: Modifier = Modifier
) {
    var isShowSubItem by remember { mutableStateOf(false) }
    ...
    Column(modifier = modifier) {
        Row(
            ...
            content(isShowSubItem)
        }
        AnimatedVisibility(visible = isShowSubItem) {
            subContent(isShowSubItem)
        }
    }
}
